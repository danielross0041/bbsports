...
class RegistrationController extends Controller
{
    ...
    public function registration_submit(request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $token_ignore = ['_token' => '','email' =>'','password'=>'','dob'=>'', 'address'=>''];
            $post_feilds = array_diff_key($_POST , $token_ignore);
            $user_feilds = [];
            $pass = $_POST['password'];
            $user_feilds['password'] = Hash::make($pass);
            $user_feilds['name'] = $_POST['firstname'] . " " . $_POST['lastname'];
            $user_feilds['username'] = strtolower($_POST['firstname']) . strtolower($_POST['lastname']);
            $user_feilds['email'] = $_POST['email'];
            $user_feilds['address'] = $_POST['address'];
            $user_feilds['dob'] = $_POST['dob'];
            $user = User::create($user_feilds);
            if ($user) {
                $post_feilds['user_id'] = $user->id;
                $post_feilds['name'] = $_POST['firstname'] . " " . $_POST['lastname'];
                $player = player::create($post_feilds);
